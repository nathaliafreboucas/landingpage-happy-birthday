  ...
  return (
    <section className="relative min-h-screen bg-linen flex flex-col overflow-hidden">
      ...
      <div aria-hidden className="absolute inset-0 pointer-events-none select-non etop-0">
        ...
        <span className="absolute top-[20%] left-[18%] text-sage   text-base  opacity-50 animate-float [animation-delay:2s]">✦</span>
        <span className="absolute top-[6%]  right-[22%] text-gold  text-sm  opacity-40 animate-float [animation-delay:0.5s]">✦</span>
      ...
      <div className="relative z-10 flex flex-col items-center justify-start flex-1 px-6 pt-16 pb-4 text-center">
        ...
        {/* Badge tema */}
        <span className="inline-block mb-4 px-4 py-1.5 rounded-full border border-sage text-sage font-sans font-semibold text-sm tracking-widest uppercase bg-sage-light">
          {theme}
        </span>

        {/* Subtítulo badge */}
        <p className="font-sans text-muted text-base tracking-wider mb-2">{badge}</p>
      ...
      <div aria-hidden className="absolute bottom-10 left-8 sm:left-16 pointer-events-none z-20">
        <HelenaSticker src="/images-person/1.png" size="lg" rotate={-12} delay="0.3s" priority />
      </div>
